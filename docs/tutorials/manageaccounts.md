...
The HD wallet manager (`@ckb-lumos/hd`) supports BIP-39 for generating seeds, mnemonic codes (12 words), extended private and public keys. 

The [mnemonic.generateMnemonic](https://github.com/nervosnetwork/lumos/blob/develop/packages/hd/src/mnemonic/index.ts#L173) function can generate 12 words mnemonic.
...
The key module of the `@ckb-lumos/hd` package supports the secp256k1 standard. The [key.privateToPublic](https://github.com/nervosnetwork/lumos/blob/c3bd18e6baac9c283995f25d226a689970dc9537/packages/hd/src/key.ts#L51) function can convert a private key to a public key.
...
### Generate the Args from a Public Key

The [key.publicKeyToBlake160](https://github.com/nervosnetwork/lumos/blob/c3bd18e6baac9c283995f25d226a689970dc9537/packages/hd/src/key.ts#L70) function uses blake160 to extracts the first 20 bytes of a public key hash as the args.
...
### Generate a Keystore File from an Extended Private Key

> A keystore file is an encrypted version of a private key. The keystore file is generated by using the private key and a password that is used to encrypt the keystore file. The ckb-cli tool or wallets like the CKB wallet, [Neuron Wallet](https://docs.nervos.org/docs/basics/guides/neuron#3-create-a-new-wallet-or-import-existing-keystore-file-or-seed-phrase-to-the-neuron-wallet), can import keystore files to create new accounts.

The [Keystore](https://github.com/nervosnetwork/lumos/blob/c3bd18e6baac9c283995f25d226a689970dc9537/packages/hd/src/keystore.ts#L55) module of the `@ckb-lumos/hd` package supports to generate keystore files from extended private keys.
...
For the keystore file that is generated by the ckb-cli tool (the keystore file contains `"origin":"ckb-cli"`), the <var>needMasterPublicKey</var> variable needs to be set as <var>true</var>, the HD cache manager loads the data by the master public key.

The following example loads the data from the keystore file of an HD wallet, and then uses the [getBalance](https://github.com/nervosnetwork/lumos/blob/c3bd18e6baac9c283995f25d226a689970dc9537/packages/hd-cache/src/index.ts#L767) function to get the balance of the HD wallet from cache. 

Example:

```typescript title="/mydapp/src/manageaccounts.ts/getBalancebyHDCache()"
...
> await manageaccounts.getBalancebyHDCache("C:\\test\\keystore","test");
The HD wallet balance is 422036350703777n
```

</p>
</details>

The HD cache manager can also load the data of HD wallets from mnemonic words with the [CacheManager.fromMnemonic](https://github.com/nervosnetwork/lumos/blob/c3bd18e6baac9c283995f25d226a689970dc9537/packages/hd-cache/src/index.ts#L605) function. 
